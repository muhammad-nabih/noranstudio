...
export default function AboutStrip() {
  ...
  useEffect(() => {
    ...
    const mkRing = (radius: number, opacity: number, x: number) => {
      const m = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0xC9A96E, transparent: true, opacity })
      );
      m.position.x = x;
      scene.add(m);
      return m;
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tlRef.current
        .to('.about-eyebrow',   { opacity: 1, y: 0,    duration: 0.8, ease: 'power3.out' })
        .to('.about-h1',        { opacity: 1, y: '0%', duration: 0.9, ease: 'power4.out' }, '-=0.4')
        .to('.about-h2',        { opacity: 1, y: '0%', duration: 0.9, ease: 'power4.out' }, '-=0.6')
        .to('.about-divider',   { width: 280,           duration: 1.2, ease: 'power3.inOut' }, '-=0.3')
        .to('.about-body',      { opacity: 1, y: 0,    duration: 0.9, ease: 'power3.out' }, '-=0.6')
        .to('.about-tagline',   { opacity: 1, y: 0,    duration: 0.7, ease: 'power3.out' }, '-=0.4')
        .to('.about-stats',     { opacity: 1, y: 0,    duration: 0.8, ease: 'power3.out' }, '-=0.3')
        .to('.about-ring',      { opacity: 1,           duration: 1.5, ease: 'power2.out' }, '-=0.8')
        .to('.about-corner-tl', { opacity: 1, x: 0, y: 0, duration: 0.7, ease: 'power3.out' }, '-=0.8')
        .to('.about-corner-br', { opacity: 1, x: 0, y: 0, duration: 0.7, ease: 'power3.out' }, '-=0.6')
        .to('.about-portrait',  { opacity: 1,           duration: 1.2, ease: 'power2.out' }, '-=0.5')
        .to('.about-glow',      { opacity: 1,           duration: 1,   ease: 'power2.out' }, '-=0.8')
        .to('.about-badge',     { opacity: 1, x: 0,    duration: 0.8, ease: 'back.out(1.4)' }, '-=0.6');

      /* shimmer loop */
      const shimmerLoop = () => {
        gsap.fromTo('.about-shimmer',
          { top: '-2px', opacity: 0 },
          { top: '102%', opacity: 0.6, duration: 2.2, ease: 'none',
