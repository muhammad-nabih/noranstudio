...
export default function MagicWandCursor() {
  ...
  // ── Mouse tracking + wand interactions ──────────────────────────────────────
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      if (now - lastSpawnTime > spawnRate) {
        ...
        for (let i = 0; i < count; i++) {
          spawnParticle(x + (Math.random() - 0.5) * 4, y + (Math.random() - 0.5) * 4, speed);
        }
        lastSpawnTime = now;
      }
      ...
      idleTimerRef.current = setTimeout(() => { isMovingRef.current = false; }, 120);
    ...
    const onMouseDown = () => {
      gsap.to(wand, { scale: 0.82, rotate: '-=3', duration: 0.1, ease: 'power3.out', overwrite: 'auto' });
    };

    const onMouseUp = () => {
      gsap.to(wand, { scale: 1, duration: 0.55, ease: 'elastic.out(1.3, 0.5)', overwrite: 'auto' });
    };
