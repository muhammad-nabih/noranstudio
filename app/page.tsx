...
// function ServiceCard({ service, campaigns, index ,project}: { service: Service; campaigns: Campaign[]; index: number ,project: Campaign}) {
//   const cardRef   = useRef<HTMLDivElement>(null)
//   const imgRef    = useRef<HTMLDivElement>(null)
//   const [hov, setHov] = useState(false)
//   const accent = SERVICE_ACCENTS[index % SERVICE_ACCENTS.length]

//   // Get campaigns that belong to this service
//   const svcCampaigns = campaigns.filter(c => c.service?._id === service._id).slice(0, 3)
...
//     const nx = ((e.clientX - left) / width  - 0.5) * 12
//     const ny = ((e.clientY - top)  / height - 0.5) * 12
...
//       className="group relative border border-[#C9A96E]/10 hover:border-[#C9A96E]/30 transition-all duration-500 overflow-hidden"
//       style={{ background: 'rgba(26,16,8,0.4)' }}
//     >
//       {/* Top accent line */}
//       <div className="absolute top-0 left-0 h-px w-0 group-hover:w-full transition-all duration-700" style={{ background: accent.color }} />

//       {/* Service number */}
//       <div
//         className="absolute top-6 right-6 text-[6rem] font-['Playfair_Display'] font-black leading-none select-none transition-all duration-500 pointer-events-none"
//         style={{ color: hov ? accent.color + '15' : '#ffffff05' }}
//       >
//         {accent.number}
//       </div>
...
//               <span className="w-4 h-px" style={{ background: accent.color }} />
//               <span className="text-xs tracking-[0.3em] uppercase font-['Cormorant_Garamond']" style={{ color: accent.dim }}>
//                 Service
//               </span>
//             </div>
//             <h3 className="text-2xl md:text-3xl font-['Playfair_Display'] font-bold text-white group-hover:text-[#C9A96E] transition-colors duration-400">
//               {service.title}
//             </h3>
//           </div>
//           {/* Arrow icon */}
//           <div
//             className="w-10 h-10 border flex items-center justify-center flex-shrink-0 transition-all duration-400 group-hover:rotate-45"
...
//         {/* Description */}
//         {service.description && (
//           <p className="text-[#E8DCC8]/55 text-sm leading-relaxed mb-8 font-['Cormorant_Garamond'] font-light">
//             {service.description}
//           </p>
//         )}

//         {/* Mini campaign previews */}
//         {svcCampaigns.length > 0 && (
//           <div className="mb-8">
//             <div className="text-xs tracking-[0.25em] uppercase text-[#E8DCC8]/30 mb-3 font-['Cormorant_Garamond']">
//               Recent Work
//             </div>
//             <div className="flex gap-2">
//               {svcCampaigns.map((c, ci) => (
//                 <Link
//                   key={c._id}
//                   href={`/campaigns/${slugOf(c)}`}
//                   className="group/thumb relative overflow-hidden flex-1"
//                   style={{ aspectRatio: '1/1.2' }}
//                 >
//                   <div ref={ci === 0 ? imgRef : undefined} className="w-full h-full">
//                     {c.heroImage ? (
//                       <Image
//                         src={getOptimizedImageUrl(c.heroImage, { width: 400, height: 480 })}
//                         alt={c.title}
//                         fill
...
//                     )}
//                     <div className="absolute inset-0 bg-[#0D0D0D]/0 group-hover/thumb:bg-[#0D0D0D]/40 transition-all duration-400" />
//                     <div className="absolute bottom-2 left-2 right-2 opacity-0 group-hover/thumb:opacity-100 transition-opacity duration-300">
//                       <p className="text-white text-xs truncate font-['Cormorant_Garamond']">{c.title}</p>
//                     </div>
//                   </div>
//                 </Link>
//               ))}
...
//           className="flex items-center gap-3 text-xs tracking-[0.25em] uppercase transition-all duration-300 font-['Cormorant_Garamond'] group/cta"
//           style={{ color: accent.color }}
//         >
//           <span className="h-px w-6 transition-all duration-400 group-hover/cta:w-12" style={{ background: accent.color }} />
...
//       {/* Bottom border flash */}
//       <div className="absolute bottom-0 left-0 h-px w-0 group-hover:w-full transition-all duration-700 delay-100" style={{ background: accent.color + '60' }} />
//     </motion.div>
//   )
// }

function ServicesSection({ services, campaigns }: { services: Service[]; campaigns: Campaign[] }) {
  const ref = useRef<HTMLDivElement>(null)
  ...
  // Derive services from campaigns if services array is empty
  ...
  return (
    <section id="services" ref={ref} className="py-32 px-8 md:px-20 max-w-[1600px] mx-auto">
      {/* Section header */}
      <div className="svc-heading mb-20 grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
        <div>
          <div className="flex items-center gap-3 mb-5">
            <span className="w-10 h-px bg-[#C9A96E]" />
            <span className="text-[#C9A96E] text-xs tracking-[0.4em] uppercase font-['Cormorant_Garamond']">What We Do</span>
          </div>
          <h2 className="text-5xl md:text-7xl font-['Playfair_Display'] font-bold text-white leading-tight">
            Our<br />
            <span style={{ WebkitTextStroke: '1px #C9A96E', color: 'transparent' }}>Services</span>
          </h2>
        </div>
        <p className="text-[#E8DCC8]/50 text-base leading-relaxed font-['Cormorant_Garamond'] font-light md:max-w-xs">
          Each discipline approached with the same obsessive attention to craft — from intimate portraiture to large-scale commercial campaigns.
        </p>
      </div>

      {/* Services grid — responsive: 1 col → 2 col → dynamic */}
      <div className={cn(
        'grid gap-6',
        derivedServices.length === 1 ? 'grid-cols-1' :
        derivedServices.length === 2 ? 'grid-cols-1 md:grid-cols-2' :
        derivedServices.length === 3 ? 'grid-cols-1 md:grid-cols-3' :
        derivedServices.length === 4 ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-2' :
        'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'
      )}>
        {derivedServices.map((svc, i) => (
          <ServiceCard key={svc._id} service={svc} campaigns={campaigns} index={i} project={campaigns[i]} />
        ))}
      </div>
    </section>
  )
